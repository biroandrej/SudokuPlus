...
class NotificationSettingsManager @Inject constructor(
    ...
    private val dailyChallengeNotificationEnabledKey = booleanPreferencesKey("daily_challenge_notification_enabled")
    private val dailyChallengeNotificationHourKey = intPreferencesKey("daily_challenge_notification_hour")
    private val dailyChallengeNotificationMinuteKey = intPreferencesKey("daily_challenge_notification_minute")
    ...
    val notificationPermissionRequested = dataStore.data.map { preferences ->
    ...
    val dailyChallengeNotificationEnabled = dataStore.data.map { preferences ->
    ...
    val dailyChallengeNotificationHour = dataStore.data.map { preferences ->
        preferences[dailyChallengeNotificationHourKey] ?: 8
    }

    val dailyChallengeNotificationMinute = dataStore.data.map { preferences ->
    ...
    // Streak reminder
    val streakReminderEnabled = dataStore.data.map { preferences ->
    ...
    val streakReminderHour = dataStore.data.map { preferences ->
        preferences[streakReminderHourKey] ?: 20
    }

    val streakReminderMinute = dataStore.data.map { preferences ->
