...
- [x] **P2** Improve backup UX: show last backup time, explicit permission/error state when SAF tree access is missing, and a retry prompt instead of silently returning `Result.failure()` (`BackupScreen`, worker logs).
...
- [x] **P3** Add accessibility labels/semantics to icon-only controls so TalkBack users can navigate: bottom bar icons (`NavigationBarComponent.kt`), difficulty/type pickers and scroll-to-top FAB (`HomeScreen.kt`), More screen quick actions (`MoreScreen.kt`), and game toolbar icons.
